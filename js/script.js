(function($) {
	...
	$(document).ready(function() {
		...
		if ($('.main-header li.dropdown ul').length) {
			...
			// Disable dropdown parent link for other menu items, not for 'About Us'
			$('.main-header .navigation li.dropdown > a, .hidden-bar .side-menu li.dropdown > a').on('click', function(e) {
				// Check if the clicked menu item is not 'About Us'
				if (!$(this).parent().hasClass('current')) {
					e.preventDefault(); // Prevent default behavior for other menu items
				}
			});
	
			// Close dropdown on outside click
			$(document).on('click', function(e) {
				if (!$('.main-header li.dropdown').is(e.target) && $('.main-header li.dropdown').has(e.target).length === 0) {
					$('.main-header li.dropdown ul').slideUp(500);
				}
			});
		}
	});
	...
	if($('.main-footer .network-column .our-branches .dropdown-menu > li').length){
		$('.main-footer .network-column .our-branches .dropdown-menu > li').on('click', function(e) {
			e.preventDefault();
			var targetText = $(this).attr('data-toggle-text');
			var targetTextBox = $('.main-footer .network-column .network-tabs .tab-btns a.btn .toggle-text');
	...
	if ($('.four-item-carousel').length) {
		$('.four-item-carousel').owlCarousel({
			...
			smartSpeed: 700,
			autoplay: 4000,
			navText: [ '<span class="flaticon-back-3"></span>', '<span class="flaticon-next-3"></span>' ],
			responsive:{
				0:{
					items:1
				},
